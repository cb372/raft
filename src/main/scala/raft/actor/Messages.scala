...
object Messages {
   ...
   */
  case class AppendEntries(term: Term, prevLogIndex: LogIndex, prevLogTerm: Term, entries: Seq[LogEntry], leaderCommit: LogIndex)
  case class AppendEntriesReply(term: Term, success: Boolean)

  /* Client-Server interaction */

  case class ExecuteCommand(command: Command)
